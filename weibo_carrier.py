...
# Theme
LIGHT_YELLOW = {'bg': '#FFFAE9', 'text': '#584A3C'}
BLUE = {'bg': '#E3F2FD', 'text': '#212121'}
PINK = {'bg': '#FCE4EC', 'text': '#212121'}
LIGHT_BROWN = {'bg': '#BCAAA4', 'text': '#FFFFFF'}
...
def count_width(i, post):
    ...
    if FONT == 'NotoSansCJKsc-Light.otf':
        ...
        if str(ord(post[i])) in width_one_one:
            ...
            width = 2
    elif FONT == 'NotoSansCJKsc-DemiLight.otf':
...
def format_post(post, formatted_post_date):
    ...
    if FONT == 'NotoSansCJKsc-Light.otf':
        for i in range(len(post)):
            if post[i] == ' ' and width_counter == 0:
                pass
            elif post[i] == ' ' and post[i] != post[-1:]:
...
def make_pic(post, formatted_post_date, post_name, weibo_name):
    ...
    if os.path.isfile(os.path.join(os.path.abspath('weibo\\%s') % weibo_name, '\\%s.jpg' % post_name)):  # 这张微博已存在
        ...
        style_three(formatted_post, formatted_post_date, line_counter, post_name, weibo_name)


def style_one(formatted_post, formatted_post_date, line_counter, post_name, weibo_name):
    if line_counter <= 10:
        width = 1080
        height = 1080
    else:
        width = 1080
        height = 1080 + (line_counter - 10) * 85
    image = Image.new('RGB', (width, height), theme['bg'])
    font_date = ImageFont.truetype(os.path.join(os.path.abspath('font'), FONT), 48)  # 'Roboto-Light.ttf'
    font_post = ImageFont.truetype(os.path.join(os.path.abspath('font'), FONT), 50)  # 24
    draw = ImageDraw.Draw(image)
    draw.text((65, 60), formatted_post_date, font=font_date, fill=theme['text'])  # (65, 65)
    draw.multiline_text((65, 190), formatted_post, font=font_post, fill=theme['text'], spacing=26, align='left')  # (65, 200) 28  4/25
    # image.show()
...
# big date material style
def style_two(formatted_post, formatted_post_date, line_counter, post_name, weibo_name):
    if line_counter <= 8:
        width = 1080
        height = 1080
    else:
        width = 1080
        height = 1080 + (line_counter - 8) * 87  # spacing=-4/25*52 not a integer, change size
    image = Image.new('RGB', (width, height), theme['bg'])
    font_date = ImageFont.truetype(os.path.join(os.path.abspath('font'), 'RobotoSlab-Thin.ttf'), 145)
    font_post = ImageFont.truetype(os.path.join(os.path.abspath('font'), FONT), 52)
    draw = ImageDraw.Draw(image)
    ...
    draw.multiline_text((20, -25), formatted_post_date, font=font_date, fill=theme['text'], spacing=-5)
    draw.multiline_text((40, 335), formatted_post, font=font_post, fill=theme['text'], spacing=26)
    post_name = post_name.replace('-', '').replace(':', '').replace(' ', '_')  # 文件名不能包含:
    image.save(os.path.join(os.path.abspath('weibo'), weibo_name) + '\\%s.jpg' % post_name, 'jpeg')


# big date material style with dark background  (recommend LIGHT_YELLOW, LIGHT_BROWN, LIGHT_GREY)
def style_three(formatted_post, formatted_post_date, line_counter, post_name, weibo_name):
    if line_counter <= 8:
        width = 1080
        height = 1080
    else:
    ...
    image = Image.new('RGB', (width, height), theme['bg'])
    font_date = ImageFont.truetype(os.path.join(os.path.abspath('font'), 'RobotoSlab-Thin.ttf'), 145)
    font_bg = ImageFont.truetype(os.path.join(os.path.abspath('font'), 'NotoSansMonoCJKsc-Regular.otf'), 1080)
    font_post = ImageFont.truetype(os.path.join(os.path.abspath('font'), FONT), 52)
    draw = ImageDraw.Draw(image)
    ...
    formatted_post_date = formatted_post_date[:-9] + '\n' + formatted_post_date[-8:-6]
    draw.multiline_text((0, 0), '█\n' * int(line_counter / 12 + 1), font=font_bg, fill=theme['text'], spacing=-173)  # (0, 450)
    draw.multiline_text((20, -25), formatted_post_date, font=font_date, fill=theme['text'], spacing=-5)
    draw.multiline_text((40, 335), formatted_post, font=font_post, fill=theme['bg'], spacing=26)
    post_name = post_name.replace('-', '').replace(':', '').replace(' ', '_')  # 文件名不能包含:
    image.save(os.path.join(os.path.abspath('weibo'), weibo_name) + '\\%s.jpg' % post_name, 'jpeg')
